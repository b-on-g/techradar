...
;
	($.$bog_techradar_app) = class $bog_techradar_app extends ($.$mol_page) {
		Theme(){
			const obj = new this.$.$bog_theme_auto();
		...
		Info_adopt(){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => (["Adopt — технологии и инструменты, которые внедрены и активно используются в командах. Технологии, по которым в X5Tech есть экспертиза."]);
			return obj;
		}
		Info_trial(){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => (["Trial — технологии и инструменты, которые уже прошли этап тестирования и готовятся к работе в продакшн (или уже работают там)."]);
			return obj;
		}
		Info_assess(){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => (["Assess — пробные технологии и инструменты, которые сейчас оцениваются. С их участием реализуются только тестовые проекты, в реальных задачах они не используются."]);
			return obj;
		}
		Info_hold(){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => (["Hold — в этой категории есть экспертиза, но инструменты используются только при поддержке существующих систем — новые проекты на них не запускаются."]);
			return obj;
		}
		Info_source(){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => (["За основу взят Tech Radar Zalando. Похожие есть у ThoughtWorks. В России концепцию используют, например, Avito и Lamoda."]);
			return obj;
		}
		Info(){
			const obj = new this.$.$mol_view();
		...
		Ring(id){
			...
			(obj.pos_x) = () => ("0");
			(obj.pos_y) = () => ("0");
			(obj.radius) = () => ((this.ring_radius(id)));
			(obj.attr) = () => ({
		...
		Axis(id){
			...
			(obj.from_x) = () => ((this.axis_from_x(id)));
			(obj.from_y) = () => ((this.axis_from_y(id)));
			(obj.to_x) = () => ((this.axis_to_x(id)));
			(obj.to_y) = () => ((this.axis_to_y(id)));
			(obj.attr) = () => ({
		...
		R_header(id){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => ([(this.r_header_text(id))]);
			return obj;
		}
		Blip_row(id){
			const obj = new this.$.$mol_link();
			(obj.uri) = () => ((this.blip_row_uri(id)));
			(obj.external) = () => (true);
			(obj.title) = () => ((this.blip_row_text(id)));
			return obj;
		}
		Blip_text(id){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => ([(this.blip_row_text(id))]);
			return obj;
		}
	};
	...
	($mol_mem(($.$bog_techradar_app.prototype), "Info_assess"));
...
(function ($) {
    ...
    (function ($$) {
        ...
        class $bog_techradar_app extends $.$bog_techradar_app {
            ...
            axis_from_x(id) { return id === 0 ? String(-MAX_AXIS) : '0'; }
        ...
        $$.$bog_techradar_app = $bog_techradar_app;
    })($$ = $.$$ || ($.$$ = {}));
})($ || ($ = {}));
